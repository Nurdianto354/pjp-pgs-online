        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active"><a href="{{ route('master_data.satuan.index') }}">Data Satuan</a></li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title"><span id="title"></span> Data Satuan</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <form name="input-data" method="POST" action="{{ route('master_data.satuan.create')}}" onsubmit="return validateForm()">
                    @csrf
                    <input type="hidden" id="id" name="id">
                    <div class="form-group">
                        <label>Satuan</label>
                        <input type="text" class="form-control" placeholder="Input satuan" id="nama" name="nama">
                    </div>
                    <small class="form-text text-danger error-nama" style="margin-top: -15px;">Harap masukan satuan !</small>
...
<script>
    ...
    $(document).on('click', '.delete-data', function(e) {
        ...
        Swal.fire({
            ...
            text: "Ingin menghapus data satuan "+nama+" ini !",
            icon: "warning",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type    : "POST",
                    url     : "{{ url('/master-data/satuan/delete') }}/" + id,
                    success: function(data) {
                        if(data.status == "success") {
                            toastMixin.fire({
                                icon: 'success',
                                title: 'Berhasil menghapus data satuan '+nama,
                            });
                            ...
                            toastMixin.fire({
                                icon: 'error',
                                title: 'Gagal, menghapus data satuan '+nama,
                            });
                        }
                    }
                });
