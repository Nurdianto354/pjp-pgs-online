                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @foreach($data->getPermissionNames() as $permission)
                                                <span class="badge badge-success p-1">{{ $permission }}</span>
                                            @endforeach
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <a href="{{ route('master_user.role.set_akses', ['id' => $data->id]) }}" class="btn btn-sm btn-success">
                                                    <i class="far fa-solid fa-user-gear"></i> Set Akses
                                                </a>
                                                ...
                                                <button type="button" class="btn btn-danger btn-sm delete-data" data-id="{{ $data->id }}" data-nama="{{ $data->name }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title"><span id="title"></span> Data Role</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <form name="input-data" method="POST" action="{{ route('master_user.role.create')}}" onsubmit="return validateForm()">
                    @csrf
                    <input type="hidden" id="id" name="id">
                    <div class="form-group">
                        <label>Role</label>
                        <input type="text" class="form-control" placeholder="Input role" id="nama" name="nama">
                    </div>
                    <small class="form-text text-danger error-nama" style="margin-top: -15px;">Harap masukan role !</small>
...
<script>
    ...
    $(document).on('click', '.delete-data', function(e) {
        ...
        Swal.fire({
            ...
            text: "Ingin menghapus data role "+nama+" ini !",
            icon: "warning",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type    : "POST",
                    url     : "{{ url('/master-user/role/delete') }}/" + id,
                    success: function(data) {
                        if(data.status == "success") {
                            toastMixin.fire({
                                icon: 'success',
                                title: 'Berhasil menghapus data role '+nama,
                            });
                            ...
                            toastMixin.fire({
                                icon: 'error',
                                title: 'Gagal, menghapus data role '+nama,
                            });
                        }
                    }
                });
